...
import { FaBrain, FaChartLine, FaRobot, FaClipboardCheck, FaTimes, FaLightbulb, FaFish, FaUpload, FaCheck, FaDownload, FaFileExport } from 'react-icons/fa';
import { downloadChart, downloadCSV, getTimestamp } from '../../utils/chartDownload';
                  ...
                  <div className="absolute top-4 right-4 flex space-x-2 z-10">
                    <button
                      onClick={() => {
                        ...
                        const analysisName = analysisNames[selectedAnalysis as keyof typeof analysisNames] || '分析';
                        downloadChart(
                          chartRef,
                          `${analysisName}_${timestamp}`,
                          'png'
                        );
                      }}
                    ...
                    <button
                      onClick={() => {
                        ...
                        const analysisName = analysisNames[selectedAnalysis as keyof typeof analysisNames] || '分析';
                        const chartData = currentChartInfo.data;
                        const csvData = chartData.labels?.map((label, index) => {
                          const row: any = { 时间: label };
                          chartData.datasets.forEach((dataset, datasetIndex) => {
                            row[dataset.label || `数据${datasetIndex + 1}`] = dataset.data[index];
                          });
                          return row;
                        }) || [];
                        downloadCSV(
                          csvData,
                          `${analysisName}_数据_${timestamp}`
                        );
                      }}
