...
import { FaDownload, FaFilter, FaSearch, FaExclamationTriangle } from 'react-icons/fa';
...
export default function WaterMonitoring() {
  console.log("WaterMonitoring组件开始渲染");
  ...
  const [waterQualityHistory, setWaterQualityHistory] = useState<WaterQualityData[]>([]);
  ...
  const waterQualityChartRef = useRef<SVGSVGElement | null>(null);
  ...
  useEffect(() => {
    ...
    // 每10秒检查一次
    const intervalId = setInterval(checkWaterQuality, 10000);
  ...
  // --- 新增：获取水质历史数据的API调用函数 ---
  const fetchWaterQualityHistory = async (pointId: string, timeRange: number) => {
    if (!pointId) return;
    ...
    try {
      ...
      for (let i = timeRange - 1; i >= 0; i--) {
        ...
        for (let hour = 0; hour < 24; hour += 6) {
          ...
          const oxygen = 6 + Math.random() * 3; // 6-9mg/L
          const turbidity = 5 + Math.random() * 15; // 5-20NTU
          
          mockData.push({
            id: `${pointId}_${timestamp.getTime()}`,
            pointId,
            timestamp,
            temperature,
            ph,
      ...
      setWaterQualityHistory(mockData);
    } catch (error) {
      console.error('获取水质历史数据失败:', error);
    } finally {
  ...
  useEffect(() => {
    if (selectedPoint) {
      fetchWaterQualityHistory(selectedPoint, timeRange);
    }
  }, [selectedPoint, timeRange]);

  // 生成时间序列数据
  const generateTimeData = (days: number): DataPoint[] => {
  ...
  // 生成产量数据
  const generateProductionData = (days: number): DataPoint[] => {
    const data: DataPoint[] = [];
    const now = new Date();
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      data.push({
        date,
        value: 100 + Math.random() * 50 + Math.sin(i * 0.2) * 20
      });
    }
  ...
  const createTemperatureChart = (data: DataPoint[]) => {
    ...
    svg.append("g")
      ...
      .call(d3.axisBottom<Date>(xScale).tickFormat(d3.timeFormat("%m/%d")));
  ...
  const createProductionChart = (data: DataPoint[]) => {
    ...
    svg.append("g")
      ...
      .call(d3.axisBottom(xScale).tickFormat((d, i) => {
        const date = data[parseInt(d)]?.date;
        return date ? d3.timeFormat("%m/%d")(date) : "";
      }));
  ...
  const createChinaMap = async () => {
    ...
    try {
      // 加载GeoJSON数据
      const response = await fetch('/geojson_full/100000.json');
      if (!response.ok) {
      ...
      // 生成模拟数据
      const generateProvinceData = (): ProvinceWaterData[] => {
      ...
      const paths = mapGroup.selectAll("path")
        ...
        .attr("fill", function(d: any): string {
          const province = provinceData.find((p: ProvinceWaterData) => p.name === d.properties.name);
        ...
        .on("mouseover", function(event: any, d: unknown) {
          ...
          const province = provinceData.find((p: ProvinceWaterData) => p.name === (d as GeoFeature).properties.name);
          if (province) {
            d3.select(this)
              .attr("opacity", 1)
              .attr("stroke-width", 1);
            ...
            const tooltip = d3.select("body").append("div")
              ...
              .html(`
                <div>${(d as GeoFeature).properties.name}</div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* 高德地图API脚本 */}
      <Script
        src="https://webapi.amap.com/maps?v=1.4.15&key=96cdd7a8bceea5f8dc25a2d8c32c98b8&callback=initAMap"
            ...
            <div className="flex items-center mb-4">
              <FaExclamationTriangle 
                className={`text-2xl mr-3 ${
                  alertStatus === 'error' ? 'text-red-500' : 'text-yellow-500'
                }`} 
              />
              <h3 className="text-lg font-semibold">
                {alertStatus === 'error' ? '严重警告' : '水质警告'}
              </h3>
            </div>
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="max-w-7xl mx-auto">
          {/* 页面标题 */}
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-900 mb-2">水质监测中心</h1>
            <p className="text-gray-600">实时监测水质状况，保障海洋生态环境</p>
          </div>
